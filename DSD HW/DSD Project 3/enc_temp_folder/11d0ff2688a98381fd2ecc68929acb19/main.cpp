...
bool read_file(ofstream& out, Board& board);

int main() {
	ofstream out;
	Board board;
	if (read_file(out, board)) {
		board.order_content();
		TwoD td(board);
		td.run(board);
	...
	if (two_in) {
		for (int i = 0; i < content.size(); i++) {
			...
			for (int i = 0; i < list.size(); i++) {
				if (list[i].next == content[i].get_inp01_next_state()) {
					str = "01/" + content[i].get_inp01_output();
					list[i].inout.push_back(str);
					find = true;
				}
			}
			if (!find) {
			...
			find = false;
			for (int i = 0; i < list.size(); i++) {
				if (list[i].next == content[i].get_inp10_next_state()) {
			...
			find = false;
			for (int i = 0; i < list.size(); i++) {
				if (list[i].next == content[i].get_inp11_next_state()) {
